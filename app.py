...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:mysqllocal@localhost/electronic'
...
def request_repair():
    if request.method == 'POST':
        ...
        new_request = RepairRequest(user_id=current_user.id,name=current_user.name, device=device, issue=issue)
